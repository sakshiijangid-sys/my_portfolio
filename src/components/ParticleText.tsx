  ...
  useEffect(() => {
    ...
    const sampleText = async () => {
      ...
      if (measuredWidth > maxTextWidth) {
        resolvedSize = Math.max(14, resolvedSize * (maxTextWidth / measuredWidth));
      ...
      if (reducedMotion) {
        ...
        gathering = false;
      } else if (hasGatheredOnScroll) {
        startGather(false);
      } else {
        // Keep particles in initial scattered position until scrolled into view
        particles.forEach(particle => {
    ...
    let hasGatheredOnScroll = false;

    const intersectionObserver = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting && !hasGatheredOnScroll) {
            hasGatheredOnScroll = true;
